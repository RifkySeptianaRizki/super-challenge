...
export default function ScheduleSection() {
  ...
  return (
    <section
      id="jadwal"
      className="relative w-full min-h-[100dvh] bg-sc-black overflow-hidden flex flex-col items-center py-14 sm:py-16"
    >
        ...
        <div className="flex flex-col items-center gap-5 sm:gap-6 w-full">
          {filteredMatches.map((day) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="border border-dashed border-gray-600 bg-black/40 backdrop-blur-md p-4 sm:p-6 w-full max-w-6xl shadow-2xl rounded-xl"
            >
              <h3 className="text-center font-bold text-lg sm:text-2xl text-white mb-4 sm:mb-5 mt-1 tracking-wide">
                {day.date}
              </h3>
              ...
              <div
                className={`grid grid-cols-1 w-full ${
                  day.games.length > 1
                    ? "lg:grid-cols-2 lg:gap-x-8 xl:gap-x-10"
                    : ""
                }`}
                style={
                  day.games.length === 1
          ...
          {filteredMatches.length === 0 && (
            <div className="text-center py-16 text-gray-400 font-medium w-full">
              ...
              <p className="text-sm mt-2 text-gray-500">
                Silakan generate bracket melalui Admin Panel.
              </p>
            </div>
          )}
        </div>
...
function MatchRow({ game, isLastDesktop, isLastMobile }) {
  ...
  const t1 = teams.find((t) => t.id === game.teamAId) || teams.find((t) => t.code === game.teamA) || {
    code: game.teamA,
    name: game.teamA,
  };

  const t2 = teams.find((t) => t.id === game.teamBId) || teams.find((t) => t.code === game.teamB) || {
    code: game.teamB,
    name: game.teamB,
  };
  ...
  return (
    <div
      className={`flex items-center justify-between border-gray-500 py-1.5 sm:py-2 ${
        isLastMobile ? "border-b-0" : "border-b border-dashed"
      } ${
        isLastDesktop ? "lg:border-b-0" : "lg:border-b lg:border-dashed"
      }`}
    >
      {/* Team 1 */}
      <div className="flex flex-col items-center w-[70px] sm:w-[90px] gap-1.5 sm:gap-2">
        <TeamLogo
          team={t1}
          code={t1.code}
          name={t1.name}
        ...
        />
        <span className="text-[11px] sm:text-xs font-bold text-gray-300 truncate w-full text-center">
          {t1.code}
        </span>
      </div>
      ...
      {/* Score 1 */}
      <div
        className="text-[3.25rem] sm:text-[4.25rem] text-white w-[50px] sm:w-[70px] text-center leading-none"
      ...
      {/* Center Details */}
      <div className="flex flex-col items-center gap-1.5 sm:gap-2 flex-1 px-2 sm:px-4">
        <span className="text-sm sm:text-base font-bold text-gray-400 tracking-wider">
          {game.time}
        </span>

        <div className="flex items-center gap-1.5 sm:gap-2">
          <span
            className={`text-xs sm:text-sm font-bold px-3 py-1.5 sm:px-4 sm:py-2 tracking-widest rounded-sm shadow-sm ${
          ...
          <span className="bg-[#E68A00] text-white text-xs sm:text-sm font-bold px-3 py-1.5 sm:px-4 sm:py-2 tracking-widest rounded-sm shadow-sm">
            BO{game.bestOf}
          </span>
        </div>

        <span className="text-xs sm:text-sm text-gray-500 font-medium tracking-wider">
          {game.matchId}
        </span>
      </div>
